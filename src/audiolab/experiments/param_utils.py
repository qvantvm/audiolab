...
def parse_param_path(path: str) -> tuple[str, str, Any]:
    if _PARAMETER_MAP_RE.match(path):
        from audiolab.graph.parameter_maps import parse_parameter_map_path
...
def get_graph_param(graph: dict[str, Any], path: str) -> Any:
    if _PARAMETER_MAP_RE.match(path):
        from audiolab.graph.parameter_maps import get_parameter_map_value
...
def set_graph_param(graph: dict[str, Any], path: str, value: Any) -> None:
    if _PARAMETER_MAP_RE.match(path):
        from audiolab.graph.parameter_maps import set_parameter_map_value
