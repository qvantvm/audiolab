"""Evaluate PASP A3–C5 register note-family model."""
...
DEFAULT_GRAPH = REPO_ROOT / "examples" / "graphs" / "pasp_register_a3_c5.json"
DEFAULT_REF_SET = REPO_ROOT / "calibration" / "examples" / "pasp_register_a3_c5_reference_set.json"
DEFAULT_OUT = REPO_ROOT / "workspace" / "experiments" / "pasp_register_a3_c5"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate PASP A3–C5 register model.")
    ...
    report_lines = [
        ...
        "",
        "## Goal",
        "Fit MIDI 57–72 as one physically coherent instrument with register-aware curves.",
        "",
        "## Model summary",
        f"- Graph: `{eval_graph_path}`",
        "- Block: `PASPNoteFamilyModel` + `PASPBridgeSoundboardModel`",
        "",
        "## Register regions",
        json.dumps(family_block.get("params", {}).get("parameterization", {}).get("registers", {}), indent=2),
        "",
        "## Parameterization",
        "See `parameter_curves.json` for evaluated curves at anchor notes.",
        "",
        "## Bridge/soundboard model",
        "Unified bridge impedance bands, parametric modal bank, radiation lowpass (`body_mix`).",
        "",
        "## Reference set",
        ...
        json.dumps(evaluation.get("by_register", {}), indent=2),
        "",
        ...
        "",
        "## Bridge/body diagnostics",
        json.dumps(evaluation.get("body_responses", []), indent=2),
        "",
        "## Worst offenders",
        ...
        "",
        "## Next experiments",
        "- Extend toward full piano with unison grouping and sympathetic resonance",
    ]
