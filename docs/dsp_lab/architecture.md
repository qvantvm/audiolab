...
Core modules under `dsp_lab.graph`, `dsp_lab.blocks`, `dsp_lab.audio`, and `dsp_lab.experiments` must not import PyQt6.

## Module map

| Module | Responsibility |
| --- | --- |
| `dsp_lab.graph` | `GraphSpec`, validation, compilation, `render_graph` |
| `dsp_lab.blocks` | `BLOCK_REGISTRY`, block implementations |
| `dsp_lab.audio` | I/O, metrics, validity gates |
| `dsp_lab.experiments` | `run_experiment`, `run_calibration_cycle`, batch render, reports |
| `dsp_lab.evaluation` | Dataset manifests, batch eval, failure clusters, regression compare |
| `dsp_lab.autoresearch` | Closed-loop cycle: cluster → hypothesis → cal → eval → decision → journal |
| `dsp_lab.governance` | Model registry, promotion gates, rollback, export |
| `dsp_lab.app` | PyQt6 graph editor (validate, render, **calibrate**) |
...
- **Dataset evaluation** (`dsp_lab.evaluation`) — manifest-scale renders, `failure_clusters.json`, agent regression reports.
- **Autoresearch** (`dsp_lab.autoresearch`) — deterministic cycle orchestration; optional advisory planner, memory, and experiment design.
- **Governance** (`dsp_lab.governance`) — candidate registration, promotion gates, active baseline, lineage.
...
2. `dsp_lab.experiments.calibration.run_calibration_cycle` reads tunables and panel rows.
