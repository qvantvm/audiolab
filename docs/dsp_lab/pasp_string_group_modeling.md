...
Configuration: `string_group_layout` in params or `StringGroupLayout` in `dsp_lab.physics.string_group_layout`.
...
`DuplexResonanceBank` in `dsp_lab.physics.pasp_piano.duplex_resonance`:
...
`SympatheticResonanceBank` in `dsp_lab.physics.pasp_piano.sympathetic_resonance`:
...
`dsp_lab.audio.metrics.string_group_metrics` — beating rate, detune spread, spectral broadening, inter-string balance, secondary resonance contribution ratios.
