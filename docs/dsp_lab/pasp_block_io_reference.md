...
Input → output relationships, parameters, and core equations for **PASP piano blocks** (`src/dsp_lab/blocks/pasp_piano.py`, physics in `src/dsp_lab/physics/pasp_piano/`).
...
Full bounds: `PASP_PARAM_BOUNDS` in `src/dsp_lab/physics/pasp_piano/params.py`.
