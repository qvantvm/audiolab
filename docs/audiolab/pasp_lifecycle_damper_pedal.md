...
[`damper.py`](../../src/audiolab/physics/pasp_piano/damper.py) increases per-mode loss via `ModalStringState.step(..., loss_multiplier)` during release—not amplitude scaling.

Parameters: `damper_engage_delay_s`, `damper_ramp_time_s`, `damper_damping_base`, `damper_damping_high`, `release_noise_level`.

## Sustain pedal

[`pedal.py`](../../src/audiolab/physics/pasp_piano/pedal.py): binary up/down with optional lift/release ramps.
