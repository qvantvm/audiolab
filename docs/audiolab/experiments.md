...
`run-experiment` (CLI `run-experiment` / `audiolab.experiments.reports.run_experiment`) and `audiolab.experiments.bundle.write_experiment_bundle` create a directory containing:
...
`BatchRenderTask` metadata drives `audiolab.experiments.batch_render.batch_render_panel` — sweep `inputs` over `panel` rows and write per-note WAVs under `out_subdir`.
