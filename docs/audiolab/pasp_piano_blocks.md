...
Internal helpers: `audiolab.physics.parameter_curves`, `audiolab.physics.note_family`, `audiolab.physics.pasp_piano/`.
...
Bounds are enforced by `clamp_pasp_param()` in `audiolab.physics.pasp_piano.params`.
...
```bash
PYTHONPATH=src python examples/run_pasp_note_example.py
PYTHONPATH=src python -m pytest tests/audiolab/test_pasp_piano_blocks.py -v
```
